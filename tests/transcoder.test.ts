...
describe("parseVideoMetadata", () => {
      ...
      streams: [
        {
          ...
          side_data_list: overrides.rotation !== undefined ? [{ rotation: overrides.rotation }] : undefined,
        },
      ],
      format: {
  ...
  test("swaps width/height for a 90 degree rotation", () => {
    const metadata = parseVideoMetadata(ffprobeJson({ width: 1920, height: 1080, rotation: 90 }));
  ...
  test("does not swap width/height for a 180 degree rotation", () => {
    const metadata = parseVideoMetadata(ffprobeJson({ width: 1920, height: 1080, rotation: 180 }));
